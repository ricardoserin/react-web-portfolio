...
function validateTicket(ticket) {
  if (!ticket.serviceId) return 'Service ID is required.';
  if (!ticket.studentId) return 'Student ID is required.';
  if (!ticket.price) return 'Price is required.';
  return '';
}

function validateService(service) {
  if (!service.name) return 'Service name is required.';
  if (service.serviceTypeId < 0) return 'Price is required.';
  return '';
}

function validateStudent(student) {
  if (!student.firstName) return 'Service first name is required.';
  if (!student.lastName) return 'Student last name is required.';
  return '';
}
...
server.post('/servicesCatalog/', (req, res, next) => {
